...
// This is just the "channel" — it doesn't hold data itself.
// null is the default value before the Provider mounts.
const AuthContext = createContext(null);

// ── 2. The Provider ─────────────────────────────────────────────────────────
// Wraps the entire app (in main.jsx) and holds the actual state.
...
export function AuthProvider({ children }) {
  ...
  // clearUser is called by the logout flow — clears state after the
  // backend deletes the cookie.
  function clearUser() {
    setUser(null);
  }

  // The value object is what every consumer (useAuth()) receives.
  // We expose setUser so SignIn/Register can store the user after login.
  const value = {
